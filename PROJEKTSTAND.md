...
## Mängel löschen – 24.09.2026

Implementiert: „Löschen“ in Mängelkarten (aktuelle Liste und Archiv) und Detailansicht. Sichtbar/ausführbar nur mit manageIssues, insbesondere Admin und entsprechend berechtigte Objektleitung; Technik bleibt ausgeschlossen. Rückfrage zeigt die konkrete Beschreibung. Entfernen aus der Oberfläche erst nach ausdrücklicher Serverbestätigung; bei Netzwerk-/Serverfehler bleibt der Eintrag sichtbar. Doppelklickschutz und Benutzerwechselprüfung. Nach Erfolg verschwinden der Mangel und seine zugehörige persönliche Planung aus der ECG-App.

Bestehendes PATCH /api/state nimmt deleteIssueId entgegen, prüft Rechte serverseitig und entfernt genau diesen Mangel sowie dessen Planungen aller Benutzer atomar mit Revisionsprüfung. Interner, nicht im State-GET ausgegebener Löschvermerk enthält ursprünglichen Eintrag, Zeit und ausführendes Konto. Wiederholungen sind idempotent; alte vollständige App-Zustände können gelöschte IDs nicht wieder einfügen. Gleichzeitige Änderungen der Mängelliste verhindern veraltete PUTs; nach Löschung eintreffende Planungsänderungen werden abgewiesen. Keine neue Serverless Function. Bereits früher ins Organisationstool übertragene Planungen werden durch diesen ECG-Löschvorgang nicht automatisch entfernt; die bestehende Schnittstelle hat keinen Löschabgleich und bleibt unverändert.

Prüfung: tests/issue-delete.mjs mit tatsächlichem Handler/UI und synthetischem Speicher bestanden: Rechte, Ablehnung/Bestätigung, Netzfehler, genau ein Zieleintrag, Planungsbereinigung, Wiederholung, unsichtbarer Löschvermerk und alte Clients. Bestehende issue-planning, time-durable, technician-maintenance und room-weekly bestanden; Inline-/Modulsyntax geprüft. Keine echten Mängel zu Testzwecken gelöscht. Vorschau und Veröffentlichung ausstehend; kein authentifizierter Produktiv-Löschtest.
