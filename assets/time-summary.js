...
function personalTimeWeekEntries(entries,userId,monday,now=Date.now()){
 ...
 return (entries||[]).filter(e=>e.userId===userId&&e.start&&e.end).map(entry=>{
  const start=Date.parse(entry.start),end=Date.parse(entry.end);
  if(!Number.isFinite(start)||!Number.isFinite(end)||end<=start||end>now||end<=from||start>=until)return null;
...
function openPersonalTimeWeek(monday){
 ...
 modal(`<h2>${timeSummaryWeekLabel(monday)}</h2><p>${timeWeekRange(monday)}</p><div class="row wrap"><button class="btn primary" onclick="exportPersonalTimeWeeks(${monday})">Woche als PDF / Drucken</button></div><div style="overflow-x:auto;margin-top:16px">${timeWeekEntryTable(rows)}</div><p class="meta">Eigene abgeschlossene Zeiten · Deutschland. Bei wochenübergreifenden Buchungen wird nur der Anteil dieser Woche angezeigt.</p><button class="btn" onclick="closeModal()">Zurück zur Übersicht</button>`);
}
