...
import {validateVoiceEntries,pendingVoiceEntries,voiceSchema} from '../lib/time-voice.js';
...
export default async function handler(req,res){
 ...
 if(req.method==='GET')return res.status(200).json({ai:!!process.env.OPENAI_API_KEY&&aiAllowed,organizationSync:!!(process.env.ORGANIZATION_API_URL&&process.env.ORGANIZATION_SYNC_TOKEN&&process.env.ORGANIZATION_SYNC_USER_EMAIL)});
 const body=req.body||{};
 try{
 ...
 }catch{return res.status(503).json({error:'Anfrage konnte nicht abgeschlossen werden. Bitte erneut versuchen. Nach einem Speicherfehler dieselbe Vorschau erneut speichern; sie wird nicht doppelt angelegt.'});}
}
